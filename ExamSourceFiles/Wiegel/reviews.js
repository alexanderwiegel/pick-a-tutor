...
// ORM - for connecting to the DB; instead of using SQL, you operate with objects
const { Sequelize } = require("sequelize");
const Op = Sequelize.Op;

// possible short-hand version of declaring a function and then exporting it
exports.getReviews = async (req, res) => {
  try {
    let reviews = await Review.findAll({
      // include expects a list of objects
      include: [
        // Course is inside a list
        { model: TutorCourse, include: [Course] },
        // basically the same as the above line, just less specific
        // TutorCourse,
...
exports.calculateNewRating = async function calculateNewRating(courseId) {
  // get the course that we want to calculate the rating for
  ...
  // we add EPSILON, multiply by 100 and then divide by 100 again to get 2 decimal places
...
exports.tutorNewRating = async function tutorNewRating(tutorId) {
  ...
  let tutorReviews = await TutorCourse.findAll({
    where: {
      UserId: tutorId,
      rating: {
        // just accept that it is in brackets
        [Op.gt]: 0,
      }
    }
  });
