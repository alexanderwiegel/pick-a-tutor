...
function ReportedReviewCard({ courseName, reviewer, reporter, reviewText, reportComment, id, handleOnAcceptOrReject }) {
  const deleteReview = async (reviewId) => {
    await apiEndPoints.deleteReview(reviewId).then(function (response) { handleOnAcceptOrReject() })
  }

  const rejectReport = async (reviewId) => {
    await apiEndPoints.rejectReport(reviewId).then(function (response) { handleOnAcceptOrReject() })
  }
